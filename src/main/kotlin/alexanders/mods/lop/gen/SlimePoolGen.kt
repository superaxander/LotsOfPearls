...
class SlimePoolGen : IWorldGenerator {
    ...
    fun generateAt(world: IWorld, x: Int, y: Int, rand: Random) {
        ...
        if (tile.name == RockBottomAPI.createInternalRes("dirt") || tile.name == RockBottomAPI.createInternalRes("grass")) {
            ...
            println(poolBlobs)
            for (b in 0..poolBlobs) this.makePool(world, x, y - 3, rand)
        }
    }

    fun makePool(world: IWorld, x: Int, y: Int, rand: Random) {
        println("making pool at $x, $y")
        for (w in -1..1)
            if (rand.nextBoolean()) {
                for (h in -1..1) if (rand.nextInt(2) != 0) world.setTile(TileLayer.MAIN, x + w, y + h, Tiles.slime) // 1 in 2 chance
            }else {
                for (h in 0..1) if (rand.nextInt(2) != 0) world.setTile(TileLayer.MAIN, x + w, y + h, Tiles.slime) // 1 in 2 chance
            }
    }
